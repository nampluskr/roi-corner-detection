# src/models/torchseg/preprocessor.py: convert standard corners into a torchvision segmentation mask target
...
class TorchsegPreprocessor(BasePreprocessor):
    """Rasterizes normalized corners as filled quad masks for torchvision segmentation training."""

    def __call__(self, corners, size=None):
        if size is None:
            raise ValueError("TorchsegPreprocessor requires the target mask size")
