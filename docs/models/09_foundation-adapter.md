...
|---|---|---|---|
| 입력 | - | (N, 3, 518, 518) | - |
| 패치 임베딩 | 14x14 패치 $\to$ 토큰 | (N, 1369, 768) | 동결 |
| Transformer x12 | attention + MLP | (N, 1369, 768) | 동결 |
| 재배열 | 토큰 $\to$ 격자 | (N, 768, 37, 37) | - |
...
```text
이미지 (3x518x518)
   $\to$ [패치 임베딩 (동결)]        $\to$ 토큰 (1369x768)
   $\to$ [Transformer x12 (동결)]    $\to$ 토큰 (1369x768)   <- 여기까지 캐싱 가능
   $\to$ [재배열]                    $\to$ 특징 맵 (768x37x37)
   $\to$ [1x1 Conv + GAP + FC (학습)] $\to$ logits (8)
   $\to$ [sigmoid]                   $\to$ 정규화 좌표 (8)
```

**(3) 특징 캐싱.** 동결 경계(그림의 "캐싱 가능" 지점)까지의 계산은 이미지마다
한 번만 수행하면 된다. 전체 데이터셋의 특징을 미리 계산해 디스크에 저장하면,
학습 루프는 저장된 특징 $\to$ head의 짧은 경로만 반복하므로 에폭당 수 초로 단축
