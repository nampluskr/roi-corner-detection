...
(seg, detect, heatmap, hybrid, line, doc, homography, foundation, gcn)의 구현 착수
...
| detect | (N, A, 5+4) 그리드 | 높음 (그리드 디코딩 + 클래스별 top-1) | 없음 | 큼 | 높음 |
...
4. hybrid: seg의 마스크 model을 재사용하고 후처리만 교체한다. seg 직후에 두면 신규 코드가
   최소가 된다.
5. detect: 그리드 박스 디코딩 후처리가 무겁고 합성 데이터 의존이 크다. 외부 의존은 없다.
...
결과 순서: homography, heatmap, seg, hybrid, detect, gcn, doc, foundation, line

## 6. 확정 순서와의 차이

- 확정 순서(도메인 적합도): seg, detect, heatmap, hybrid, line, doc, homography, foundation, gcn
- 제안 순서(난이도/속도): homography, heatmap, seg, hybrid, detect, gcn, doc, foundation, line
...
순서: homography, seg, hybrid, heatmap, detect, gcn, doc, foundation, line
...
채택 순서: direct -> homography -> heatmap -> seg -> hybrid -> detect -> gcn -> doc
-> foundation -> line
