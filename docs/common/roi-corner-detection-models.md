...
- **preprocessor 타깃**: corners -> 채운 볼록 사각형 마스크 (preprocessor 내부 torch
...
- **preprocessor 타깃**: corners -> 채운 볼록 사각형 마스크 (`seg`와 동일)
- **loss**: BCE/Dice (mask vs 마스크 타깃)
- **후처리**: findContours + approxPolyDP -> (N, 4, 2) (`seg` postprocessor 재사용)
...
- **preprocessor 타깃**: corners -> 코너별 소형 박스 + 클래스(코너 인덱스), torchvision
  target dict 형식
