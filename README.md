...
|---|---|---:|---|---|---|
| `direct` | Direct Coordinate Regression (카탈로그 A) | 1 | ResNet backbone $\to$ GAP 또는 공간 보존 head $\to$ 8개 좌표 회귀 | 없음 | 낮음 |
| `homography` | Homography Regression (카탈로그 E) | 2 | 공간 보존 head로 정준 사각형 기준 8개 bounded offset 회귀 | 없음 | 낮음 |
...
| `doc` | Document-oriented Regression (카탈로그 J 대응) | 8 | ImageNet ResNet + 공간 보존 head, warmup 후 fine-tuning | 없음 | 높음 |
| `foundation` | Foundation Adapter (카탈로그 H) | 9 | frozen DINOv2 patch grid + 경량 공간 보존 head | `timm`, DINOv2 가중치 | 높음 |
| `line` | Line Intersection (카탈로그 D) | 10 | M-LSD MobileNetV2/FPN tp-map + 직선 그룹화 및 교점 계산 | M-LSD 가중치 | 중간 |
...
두고, 같은 좌표 회귀 계열인 `homography`와 외부 의존이 없는 `heatmap`을 앞에 둔다. `seg`는
기존에 `hybrid`(I)의 세그멘테이션 단계로 흡수되어 있던 카탈로그 C를 독립 방법론으로 승격한
것이고(그래서 `seg` 직후 `hybrid`가 동일한 마스크 model을 독립 구현한다), `det`는 카탈로그 G
(DETR-style)가 아닌 바운딩박스 기반 keypoint detector로 새로 설계한 방법론이다. 전용
외부 가중치 또는 패키지 의존이 있는 `foundation`, `line`은 뒤로 둔다. `doc`은 카탈로그 J의
few-shot 적응 의도를 계승하지만, 현재는 DocTr/DocScanner가 아닌 ImageNet pretrained ResNet을
사용한다. 마지막의 `torchseg`,
...
표의 12개 방법론은 모두 전용 4파일과 `factory.get_wrapper()` 등록까지 완료된 상태다. 이는
구조 구현과 공통 파이프라인 연결의 완료를 뜻하며, 동일한 학습 조건에서의 전체 benchmark와
모델별 성능 안정화는 별도 단계다. 특히 `line`은 target, loss, 직선 그룹화 후처리의 수렴성을
우선 검증한다.
...
| `doc` | (N, 8) 좌표 logits (ImageNet ResNet + 공간 보존 head) | 정규화 좌표 그대로 | logits vs 좌표 타깃 | sigmoid + reshape (N,4,2) |
| `foundation` | (N, 8) 좌표 logits (frozen DINOv2 patch grid + 경량 head) | 정규화 좌표 그대로 | logits vs 좌표 타깃 | sigmoid + reshape (N,4,2) |
| `line` | (N, 16, 256, 256) M-LSD tp-map | corners $\to$ dense center/displacement map | Focal + masked SmoothL1 | 직선 그룹화 + 교점 계산 $\to$ (N,4,2) |
...
`direct`, `heatmap`, `doc`, `foundation`은 기본 `compute_losses()`를 사용한다. `homography`,
`seg`, `hybrid`, `det`, `gcn`, `line`, `torchseg`은 방법론별 `compute_losses()`만
오버라이드하고 공통 step을 사용한다. `torchdet`만 torchvision detector의 호출 규약 때문에
`train_step`/`eval_step`/`predict_step`을 오버라이드한다. 누적 결과 조회는
`get_loss_results()`/`get_metric_results()`로 분리한다.
...
class DirectWrapper(BaseWrapper):
    def __init__(self, backbone="resnet50", head_type="gap", optimizer=None, scheduler=None,
...
- `line/`: `LineModel`(M-LSD MobileNetV2/FPN tp-map), `LinePreprocessor`(corners$\to$dense center/displacement map),
  `LinePostprocessor`(직선 그룹화+교점 계산, 직선 교점 연산은 이 폴더 내부에 둠), `LineWrapper`(Focal + masked SmoothL1, encoder warmup)
- `doc/`: `DocModel`(ImageNet pretrained ResNet + 공간 보존 head), `DocPreprocessor`(direct와 동일 방식),
  `DocPostprocessor`(direct와 동일 방식), `DocWrapper`(Wing Loss, encoder warmup)
- `foundation/`: `FoundationModel`(frozen DINOv2 patch grid + 공간 보존 head),
  `FoundationPreprocessor`/`FoundationPostprocessor`(direct와 동일 방식), `FoundationWrapper`(head-only Wing Loss)
...
def get_wrapper(method, device=None, **kwargs): ...   # method: direct|homography|heatmap|seg|hybrid|det|gcn|doc|foundation|line|torchseg|torchdet
...
이 프로젝트는 **레거시 프로젝트**(`../260622_roi-corner-detection-ver0`)의
