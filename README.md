...
| SR (Success Rate) | 4점 검출 성공 표본 비율. 후처리 실패 모드가 있는 방법론(seg/detect/line/hybrid)의 공정 비교 필수 지표 |
| PCK@tau | MaxCD가 임계값 tau 이하인 표본 비율 (tau는 픽셀 기준, 기본 2px/5px) |
| GPU/CPU latency, 모델 크기 | 배포 적합성 비교 (`scripts/benchmark.py`). latency는 전처리 -> 추론 -> 후처리 end-to-end 기준 |

모든 메트릭은 `src/metrics/metrics.py`에 위치하며, 각 방법론의 `postprocessor.py`가 만든
표준 코너 좌표(`(N, 4, 2)`)에 대해 방법론 이름과 무관하게 동일하게 적용된다.
정확도 메트릭(IoU, MCD, MaxCD, RE, PCK)은 검출 성공 표본에 대해 계산하고 항상 SR과
함께 보고한다. SR은 postprocessor의 성공 플래그를 Evaluator가 집계하는 지표이다.
...
class Dataset(torch.utils.data.Dataset):
    ...
    # transform 생략 시 ToTensor()가 기본 적용되어 항상 (image, corners) 텐서 반환
    def _load_csv(self, csv_path): ...   # -> [(image_path, corners), ...] 리스트, corners: (4, 2) np.float32
    def split(self, split_ratio=0.8, seed=42): ...
    # -> (train_subset, valid_subset), torch.utils.data.random_split 기반 Subset 반환
    def __len__(self): ...
    def __getitem__(self, idx): ...   # -> (image, corners), self.transform(image, corners) 적용 결과
```
...
# 이미지 전용 변환 - corners는 그대로 통과
class ColorJitter:
    def __init__(self, brightness=0, contrast=0, saturation=0, hue=0): ...
class GaussianBlur:
    def __init__(self, kernel_size, sigma=(0.1, 2.0)): ...
class ToTensor:
    def __call__(self, image, corners): ...   # -> (image_tensor, corners_tensor) 둘 다 torch.float32
class Normalize:
...
def create_data(data_dir, output_path): ...   # midv2020.py: data_dir 아래 masks/ + images/ 구조 기대,
                                               # output_path에 gt_corners.csv 저장 (mask -> corners, 반환값 없음)
```
...
def pck(pred_corners, gt_corners, tau): ...      # -> bool, MaxCD <= tau 표본 단위 성공 여부
# SR(Success Rate)은 metrics.py 함수가 아니라 postprocessor 성공 플래그를 Evaluator가 집계하는 지표
```
