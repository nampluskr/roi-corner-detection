...
그대로 사용한다. `BaseWrapper`가 공통 `train_step`/`eval_step`과 scheduler를 제공하며,
방법론별 loss 계산 차이는 `compute_losses()` 오버라이드로 처리한다. torchvision detector처럼
...
class BaseLoss:
    ...
    def __call__(self, raw_output, target): ...   # forward 호출 + 자동 누적, $\to$ scalar loss
