...
class Dataset(torch.utils.data.Dataset):
    ...
    # -> (train_subset, valid_subset), 각각 Subset 인스턴스
    def subset(self, num_samples, seed=42): ...  # -> Subset, 무작위 num_samples개
    def set_transform(self, transform): ...  # -> transform만 교체한 새 Dataset 인스턴스(불변)
    def __len__(self): ...
...
class Subset(torch.utils.data.Dataset):
    ...
    def set_transform(self, transform): ...  # -> 같은 인덱스, 새 dataset을 가진 새 Subset(불변)
    def subset(self, num_samples, seed=42): ...  # -> Subset, 무작위 num_samples개 (split 결과에서도 재사용 가능)
    def __len__(self): ...
    def __getitem__(self, idx): ...

class CornerDataset(Dataset):
    # x1..y4 코너 컬럼이 있는 CSV용. _parse_row -> (image_path, corners), corners: (4, 2) np.float32
    def __getitem__(self, idx): ...  # -> (image, corners), self.transform(image, corners) 적용 결과

class ImageDataset(Dataset):
    # 코너 컬럼이 없는 CSV용. _parse_row -> image_path
    def __getitem__(self, idx): ...  # -> image, self.transform(image) 적용 결과
```
...
class ToTensor:
    def __call__(self, image, corners=None): ...
    # -> corners 없으면 image_tensor만, 있으면 (image_tensor, corners_tensor) 둘 다 torch.float32
class Normalize:
...
class ToNumpy:
    def __call__(self, image, corners=None): ...
    # image: (3, H, W) 텐서 -> clamp(0,1) 후 (H, W, 3) numpy. corners는 주어지면 .numpy()로 변환
class GaussianNoise:
